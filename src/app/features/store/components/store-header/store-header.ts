...
@Component({
  ...
  template: `
    ...
    >
      <div
        class="mx-auto px-4 sm:px-6 lg:px-8 h-16 md:h-20 flex items-center gap-4"
      ...
      >
        ...
        @if (logoPosition() !== 'center') {
          ...
          >
            ...
            <span
              class="text-xl font-extrabold tracking-tight group-hover:opacity-80 transition-opacity"
      ...
      @if (mobileMenuOpen()) {
        <div
          class="border-t border-slate-100 px-4 py-4 space-y-1 animate-in slide-in-from-top-2 duration-200"
          [class.md:hidden]="!isHamburger()"
          [style.background-color]="
            branding()?.header_color || 'var(--store-color-header, #ffffff)'
          "
        >
          @for (link of navigation(); track link.label) {
            <a
    ...
    <ng-template #actionsTemplate>
      ...
      @if (user()) {
        ...
        <div
          class="h-9 w-9 rounded-2xl bg-black/5 dark:bg-white/10 flex items-center justify-center border border-black/5 shadow-xs"
      ...
      @if (showCart()) {
        ...
        >
          ...
          @if (cartCount() > 0) {
            <span
              class="absolute -top-1 -right-1 h-5 w-5 text-white text-[10px] font-black flex items-center justify-center rounded-full border-2 border-white shadow-xs"
...
export class StoreHeader {
  ...
  readonly navigation = computed(
    () => this.tenantService.publishedStorefrontLayout().navigation || [],
  );
  ...
  readonly primaryContrastColor = computed(() => {
    return getContrastColor(
      this.branding()?.primary_color || this.themeTokens()?.colors?.primary || '#0a0a0a',
    );
  });
