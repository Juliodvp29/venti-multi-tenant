...
export class ProductForm implements OnInit, OnDestroy {
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    is_featured: [false],
    variants: this.fb.array([], this.uniqueSkuValidator()), // FormArray for variants
  });
  ...
  generateVariantsFromOptions() {
    ...
    const generateLevel = (
      ...
      def.values.forEach((val) => {
        results = results.concat(
          generateLevel(level + 1, { ...currentOptions, [def.name]: val }, [...currentNames, val]),
        );
      });
      return results;
    };
    ...
    newVariants.forEach((nv) => {
      ...
      const existing = currentVariants.find(
        (cv) => JSON.stringify(cv.options) === JSON.stringify(nv.options),
      );
  ...
  uniqueSkuValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      const formArray = control as FormArray;
      if (!formArray || !formArray.controls.length) return null;

      const skus = formArray.controls
        .map((group) => group.get('sku')?.value?.toString().trim().toLowerCase())
        .filter((sku) => sku && sku.length > 0);

      const hasDuplicates = skus.some((sku, index) => skus.indexOf(sku) !== index);

      return hasDuplicates ? { duplicateSku: true } : null;
    };
  }
}
