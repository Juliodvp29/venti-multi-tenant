...
export class OrderDetail implements OnInit {
  ...
  readonly customerFullName = computed(() => {
    ...
    if (!o) return '';
    return (
      [o.customer_first_name, o.customer_last_name].filter(Boolean).join(' ') || o.customer_email
    );
  });
  ...
  readonly isBillingSameAsShipping = computed(() => {
    ...
    if (!o) return false;
    return (
      o.billing_address_line1 === o.shipping_address_line1 && o.billing_city === o.shipping_city
    );
  });
  ...
  async updateStatus() {
    ...
    try {
      ...
      this.toast.success(
        `Estado actualizado a "${this.statusOptions.find((s) => s.value === newStatus)?.label}".`,
      );
    } catch (error: any) {
  ...
  getStatusIcon(status: OrderStatus): string {
    const icons: Partial<Record<OrderStatus, string>> = {
      ...
      [OrderStatus.Paid]:
        'M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33',
      [OrderStatus.Shipped]:
