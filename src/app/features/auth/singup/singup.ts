...
@Component({
  ...
  template: `
    <div
      class="min-h-screen bg-slate-50 dark:bg-gray-950 flex flex-col items-center justify-center p-6 relative overflow-hidden"
    >
      <!-- Background Orbs -->
      <div class="absolute inset-0 overflow-hidden pointer-events-none">
        <div
          class="absolute -top-[250px] -left-[250px] w-[500px] h-[500px] rounded-full bg-sky-500/10 blur-[80px]"
        ></div>
        <div
          class="absolute -bottom-[200px] -right-[200px] w-[400px] h-[400px] rounded-full bg-sky-500/10 blur-[80px]"
        ></div>
      </div>
      ...
      <div class="relative z-10 mb-10 text-center">
        ...
        <p
          class="text-[10px] font-bold uppercase tracking-[0.3em] text-gray-500 dark:text-gray-400"
        >
      ...
      <div
        class="relative z-10 max-w-lg w-full bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-2xl overflow-hidden animate-slide-up p-8 md:p-12"
      >
        @if (isSuccess()) {
          <div class="flex flex-col items-center justify-center text-center py-8 animate-fade-in">
            <div
              class="w-20 h-20 bg-sky-50 dark:bg-sky-500/10 rounded-full flex items-center justify-center text-sky-600 mb-8 scale-up"
            >
          ...
          <div class="mb-10 text-center">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">
              {{
                isInviteFlow()
                  ? 'Completa tu registro para unirte a la tienda.'
                  : 'Configura tu cuenta de administrador para tu nueva tienda.'
              }}
            </p>
          </div>
          ...
          @if (errorMessage()) {
            <div
              class="mb-8 p-4 bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 text-red-600 dark:text-red-400 rounded-2xl text-[11px] font-bold uppercase flex items-center gap-3"
            >
          ...
          <form [formGroup]="signupForm" (ngSubmit)="onSubmit()" class="space-y-6">
            @if (!isInviteFlow()) {
              <div class="space-y-2">
                ...
                @if (businessNameError()) {
                  <p class="text-[10px] text-red-500 font-bold px-1">{{ businessNameError() }}</p>
                }
              </div>
            }
            ...
            <div class="space-y-2">
              ...
              <input
                ...
                placeholder="admin@venti.com"
                class="w-full bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700/50 rounded-2xl px-5 py-4 text-sm font-medium focus:ring-2 focus:ring-sky-500 outline-none transition-all dark:text-white"
              />
              @if (emailError()) {
                <p class="text-[10px] text-red-500 font-bold px-1">{{ emailError() }}</p>
              }
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2">
                ...
                @if (passwordError()) {
                  <p class="text-[10px] text-red-500 font-bold px-1">{{ passwordError() }}</p>
                }
              </div>
              <div class="space-y-2">
                <label
                ...
                @if (confirmPasswordError()) {
                  <p class="text-[10px] text-red-500 font-bold px-1">
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (token) {
        ...
        this.toast.warning(
          ...
          'Para registrarte, debes ser invitado o seleccionar un plan de nuestra página de inicio.',
        );
  ...
  async onSubmit() {
    ...
    if (token) {
      try {
        ...
        const { error: acceptError } = await (this.supabase.client.rpc as any)(
        ...
        if (acceptError) {
          ...
          this.toast.success(
            '¡Bienvenido!',
            'Tu cuenta fue creada y te has unido exitosamente a la tienda.',
          );
        }
