...
@Component({
  ...
  template: `
    <div
      class="fixed inset-0 z-[60] flex items-center justify-end bg-black/50 backdrop-blur-sm px-4 py-6 sm:p-0"
    >
      <div
        class="w-full max-w-lg bg-white dark:bg-gray-900 h-full overflow-y-auto shadow-2xl flex flex-col transform transition-transform duration-300"
      >
        <!-- Header -->
        <div
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="flex-1 p-6 space-y-8">
          ...
          <section class="space-y-4">
            <div
              class="flex items-center gap-2 text-sky-600 dark:text-sky-400 font-semibold text-xs tracking-wider uppercase"
            >
          ...
          <div class="p-4 bg-sky-50 dark:bg-sky-900/20 rounded-xl flex gap-3">
            <svg class="w-5 h-5 text-sky-600 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
              <path
                fill-rule="evenodd"
            ...
            <p class="text-xs text-sky-800 dark:text-sky-300 leading-relaxed">
              Este cupón estará disponible para todos los clientes una vez publicado. Puedes
              restringirlo a grupos de clientes específicos en la configuración.
            </p>
          </div>
        </form>
        ...
        <!-- Footer -->
        <div
          class="p-6 border-t border-gray-200 dark:border-gray-800 flex gap-3 pb-32 sm:pb-20 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md sticky bottom-0"
        >
          <button
            type="button"
            (click)="close.emit()"
...
export class CouponModalComponent implements OnInit {
    ...
    if (existingCoupon) {
      this.form.patchValue({
        ...
        starts_at: existingCoupon.starts_at
          ? new Date(existingCoupon.starts_at).toISOString().split('T')[0]
          : '',
        ends_at: existingCoupon.ends_at
          ? new Date(existingCoupon.ends_at).toISOString().split('T')[0]
          : '',
      } as any);
    }
  }
  ...
  async onSubmit() {
    ...
    try {
      ...
      if (existingCoupon) {
        await this.discountsService.updateDiscountCode(
          existingCoupon.id,
          data as Partial<DiscountCode>,
        );
