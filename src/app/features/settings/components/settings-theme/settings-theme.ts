...
import {
  ...
  ButtonStyleOption,
  CardStyleOption,
  CardOrientationOption,
  CardBorderStyleOption,
  CardCartButtonOption,
  HeaderStyleOption,
  HeroStyleOption,
  SpacingDensityOption,
  MaxContentWidthOption,
  TypographyPairing,
  FontWeightOption,
  BaseFontSizeOption,
  LineHeightOption,
  ...
  NavSpacingOption,
  FooterColumnsOption,
  FooterThemeModeOption,
  FooterAlignmentOption,
  FooterPaymentMethod,
  FooterLegalLink,
  CustomThemePreset,
} from '@core/models';
...
import {
  ...
  highlightCssToHtml,
  CSS_VARIABLE_CATALOG,
  CSS_SNIPPETS,
  CssValidationMessage,
  CssVariableCatalogGroup,
  CssSnippet,
} from '@core/utils/css-validator';
...
export class SettingsTheme {
  ...
  readonly logoPositionOptions: {
    label: string;
    value: LogoPositionOption;
    icon: string;
    description: string;
  }[] = [
    {
      ...
      value: 'left',
      icon: '◀',
      description: 'Logo y nombre alineados a la izquierda (clásico)',
    },
    {
      label: 'Centro',
      value: 'center',
      icon: '●',
      description: 'Logo centrado con menú dividido a ambos lados',
    },
    {
      label: 'Derecha',
      value: 'right',
      icon: '▶',
      description: 'Acciones a la izquierda, logo a la derecha',
    },
  ];

  readonly navAlignOptions: {
    label: string;
    value: NavAlignOption;
    icon: string;
    description: string;
  }[] = [
    ...
    { label: 'Centro', value: 'center', icon: '●', description: 'Menú horizontalmente centrado' },
    {
      ...
      value: 'right',
      icon: '▶',
      description: 'Menú hacia los elementos de acción',
    },
  ];
  ...
  readonly activeCustomizerTab = signal<
    | 'preset'
    | 'colors'
    | 'typography'
    | 'borders'
  ...
  readonly filteredCssVariables = computed(() => {
    ...
    return this.cssVariableCatalog
      .map((group) => {
    ...
    [
      {
        label: 'Compacto',
        value: 'compact',
        description: 'Menor espaciado, mayor densidad de productos en pantalla',
      },
      { label: 'Normal', value: 'normal', description: 'Espaciado equilibrado estándar' },
      {
        label: 'Espacioso',
        value: 'spacious',
        description: 'Diseño editorial con respiración generosa',
      },
    ];

  readonly maxContentWidths: {
    label: string;
    value: MaxContentWidthOption;
    description: string;
  }[] = [
    { label: 'Compacto (1024px)', value: '1024px', description: 'Diseño estrecho enfocado' },
    {
      label: 'Estándar (1280px)',
      value: '1280px',
      description: 'Ancho tradicional para la mayoría de tiendas',
    },
    {
      label: 'Amplio (1440px)',
      value: '1440px',
      description: 'Layout moderno aprovechando pantallas grandes',
    },
    {
      label: 'Pantalla Completa (100%)',
      value: '100%',
      description: 'Diseño fluido sin márgenes rígidos',
    },
  ];
  ...
  readonly shadowStyles: { label: string; value: ShadowStyleOption; description: string }[] = [
    ...
    {
      label: 'Sombra Dura Retro',
      value: 'hard',
      description: 'Estilo brutalista con sombra solida',
    },
  ...
  readonly cardStyles: { label: string; value: CardStyleOption; description: string }[] = [
    {
      label: 'Minimalista sin borde',
      value: 'minimal',
      description: 'Limpio y flotante sobre el fondo',
    },
    ...
    { label: 'Glassmorphism', value: 'glass', description: 'Fondo translúcido con efecto cristal' },
    {
      ...
      value: 'magazine',
      description: 'Detalles con marco de revista',
    },
  ...
  readonly footerColumnOptions: {
    label: string;
    value: FooterColumnsOption;
    icon: string;
    description: string;
  }[] = [
    {
      ...
      value: '1',
      icon: '▐',
      description: 'Centrado simple, ideal para tiendas pequeñas',
    },
    {
      label: '2 Columnas',
      value: '2',
      icon: '▐▐',
      description: 'Logo + Navegación, limpio y equilibrado',
    },
    {
      label: '3 Columnas',
      value: '3',
      icon: '▐▐▐',
      description: 'Estándar: Logo, Navegación y Contacto',
    },
    {
      label: '4 Columnas',
      value: '4',
      icon: '▐▐▐▐',
      description: 'Completo con Newsletter integrado',
    },
  ];

  readonly footerThemeModeOptions: {
    label: string;
    value: FooterThemeModeOption;
    description: string;
  }[] = [
  ...
  readonly footerAlignmentOptions: {
    label: string;
    value: FooterAlignmentOption;
    description: string;
  }[] = [
  ...
  isTypographyPairingActive(pairing: TypographyPairing): boolean {
    ...
    return (
      current.font_heading === pairing.font_heading &&
  ...
  selectPreset(presetId: ThemePresetId) {
    ...
    this.toastService.info(
      `Tema "${preset.name}" aplicado. Se guardará automáticamente en tu borrador.`,
    );
  }
  ...
  applyTypographyPairing(pairing: TypographyPairing) {
    ...
    this.emitChanges();
    this.toastService.info(
      `Combinación tipográfica "${pairing.name}" aplicada. Se guardará automáticamente en tu borrador.`,
    );
  }
  ...
  async save(silent = false): Promise<boolean> {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (!silent) this.toastService.success('Sistema visual y tema guardados exitosamente');
        this.dirtyChange.emit(false);
        return true;
      } else {
        if (!silent) this.toastService.error(result.error || 'Error al guardar el tema visual');
        return false;
      }
    } catch {
      if (!silent) this.toastService.error('Error al guardar la configuración visual');
      return false;
    } finally {
  ...
  insertCssVariable(variable: string) {
    ...
    const updated =
      current + (current.endsWith(' ') || current.endsWith(':') ? '' : ' ') + toInsert;
    this.updateCustomCss(updated);
