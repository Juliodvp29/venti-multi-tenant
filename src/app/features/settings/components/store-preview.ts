...
import { getMockProductsForIndustry, MockProductItem } from '@core/constants/industry-products';
...
export class StorePreview {
  ...
  readonly pageBgColor = computed<string>(() => {
    return (
      this.activePageConfig()?.styles?.backgroundColor || this.data().background_color || '#ffffff'
    );
  });

  get mockProducts(): MockProductItem[] {
    if (this.data().mock_products && this.data().mock_products!.length > 0) {
      return this.data().mock_products!;
    }
    return getMockProductsForIndustry(this.data().industry);
  }
  ...
  readonly dynamicCssVars = computed(() => {
    ...
    if (bgImage) {
      ...
      bgStyles = {
        'background-image':
          'radial-gradient(at 0% 0%, rgba(99, 102, 241, 0.12) 0px, transparent 50%), radial-gradient(at 100% 100%, rgba(236, 72, 153, 0.12) 0px, transparent 50%)',
      };
  ...
  get footerMutedColor(): string {
    ...
    if (mode === 'custom') {
      const isDark =
        getContrastColor(this.activeTokens.footer_custom_bg || '#0f172a') === '#ffffff';
  ...
  get footerDescription(): string {
    return (
      this.activeTokens.footer_description ||
  ...
  get footerNewsletterDescription(): string {
    return (
      this.activeTokens.footer_newsletter_description ||
  ...
  get footerPaymentMethods(): string[] {
    return (
      this.activeTokens.footer_payment_methods || [
        'visa',
        'mastercard',
        'amex',
        'paypal',
